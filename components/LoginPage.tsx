  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const userProfile = {
        ...
        role: 'Administrateur',
        jobTitle: `Gérant - ${registerData.activity}`,
      ...
      <div className="w-full lg:w-1/2 flex flex-col justify-center items-center p-8 md:p-16 overflow-y-auto">
        <div className="w-full max-w-md space-y-8 py-10">
          ...
          <div className="space-y-2">
            <h3 className="text-3xl font-bold text-gray-900 tracking-tight">{view === 'login' ? 'Ravi de vous revoir' : 'Créer votre compte Xora'}</h3>
            <p className="text-gray-500 font-medium">{view === 'login' ? 'Accédez à votre tableau de bord.' : 'Initialisez votre société.'}</p>
          </div>
          {error && (
          ...
          {view === 'login' ? (
            ...
            <form onSubmit={handleRegister} className="space-y-6 animate-in fade-in slide-in-from-left-4 duration-300 pb-10">
              ...
              <div className="space-y-4 pt-4">
                <div className="flex items-center gap-2 text-indigo-500"><User size={16} /><span className="text-[11px] font-black uppercase tracking-widest">Compte Administrateur</span></div>
                <div className="grid grid-cols-2 gap-4">
              ...
              <div className="flex gap-4 pt-4">
                ...
                <button type="submit" disabled={isLoading} className="flex-[2] bg-gray-900 text-white rounded-2xl py-4 font-bold text-sm shadow-xl hover:bg-black transition-all flex items-center justify-center gap-2 disabled:opacity-70">{isLoading ? <Loader2 className="animate-spin" size={20} /> : <><CheckCircle2 size={18} /> Créer mon espace</>}</button>
              </div>
            </form>
          )}
        </div>
